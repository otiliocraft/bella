...
// --- MODALES REGISTRO Y RECUPERAR ---
  ...
  if (olvideLink && modalRecuperar) {
    olvideLink.addEventListener('click', function(e) {
     ...
     const recuperarExito = document.getElementById('recuperar-exito');
if (recuperarExito) {
  recuperarExito.style.display = 'none';
}
    });
  }
...
function mostrarMensajeExito(mensaje = '✅ ¡Operación realizada con éxito!') {
  const mensajeDiv = document.getElementById('mensaje-exito');
  if (!mensajeDiv) {
    console.error("No se encontró el elemento #mensaje-exito");
    return;
  }

  // Actualizar texto si es necesario
  const parrafo = mensajeDiv.querySelector('p');
  if (parrafo) {
...
// Cerrar mensaje al hacer clic fuera o en la X
document.addEventListener('click', function cerrarMensaje(e) {
